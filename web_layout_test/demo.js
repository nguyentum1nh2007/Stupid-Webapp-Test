...
function deleteAllBinData() {
    const confirmDelete = confirm("Are you sure you want to empty the recycle bin?");
    if (!confirmDelete) {
        return;
    }
    // openConfirmationBox();
...
function deleteGroup(event, groupName) {
    ...
    // Show confirmation pop-up
    const confirmDelete = confirm(`Are you sure you want to delete the group "${groupName}" and all of its tasks?`);
    if (!confirmDelete) {
        return;
    }
    // openConfirmationBox();

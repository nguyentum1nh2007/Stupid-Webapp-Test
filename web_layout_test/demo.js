...
function renderGroups() {
    ...
    filteredGroups.forEach(group => {
        ...
        li.innerHTML = `<span>${group}</span> <span class="deleteGroupBtn" onclick="deleteGroup(this)">X</span>`;
        li.onclick = () => {
            currentGroup = group;
            renderGroups();
            renderTasks();
...
function renderRecycleBin() {
    ...
    recycleBin.forEach((task, index) => {
        ...
        item.innerHTML = `
            ...
            <button class="btn btn-yellow" style="padding: 4px 10px; font-size: 0.8rem;" onclick="restoreTask(${index})">Restore</button>
        `;
...
function deleteGroup(element) {
    var test = element.textContent;
    let parent = element.parenrNode;
